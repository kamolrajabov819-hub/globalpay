(function () {
    ...
    /* ================= HELPER: ROBUST LANGUAGE DETECTION ================= */
    function getLanguageContent() {
        // We check the path specifically to avoid errors with query params
        ...
        // Logic: Check if path starts with /uz or /en, or contains /uz/ /en/
        // Matches: /uz, /uz/dashboard, /app/uz, etc.
        if (path.includes('/uz') || path === 'uz') return content.uz;
        if (path.includes('/en') || path === 'en') return content.en;
        
        // Default to Russian
        return content.ru;
    }

    const currentLang = getLanguageContent();
    ...
    widgetStyles.textContent = `
        ...
        .chat-window {
            position: fixed;
            bottom: 70px; /* Lowered to match smaller button */
            right: 20px;
            width: 340px; 
            height: 480px;
        ...
        /* --- UPDATED SMALLER LAUNCHER BUTTON --- */
        .chat-launcher {
            position: fixed;
            bottom: 20px;
            right: 20px;
    ...
    launcher.textContent = currentLang.launcher;
        ...
        try {
            const res = await fetch(settings.webhook.url, {
                ...
                body: JSON.stringify({
                    ...
                    // Sending language code to backend if needed
            ...
            // PRIORITY: If server returns empty, use Local Welcome
            msg.textContent = (data && data.output) ? data.output : settings.branding.welcomeText;
            messages.appendChild(msg);
        } catch (err) {
            hideTyping();
            const msg = document.createElement('div');
            msg.className = 'chat-bubble bot-bubble';
            msg.textContent = settings.branding.welcomeText;
            messages.appendChild(msg);
        }
        scrollToBottom();
    }
